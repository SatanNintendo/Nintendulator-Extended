/*
  ...
  This is the standard client API implemented by every kailleraclient.dll
  build, including:
    - original Kaillera 0.9 client
    - SupraClient / SupraclientC (God-Weapon)
    - Open Kaillera
    - AQZ Netplay Client
    - all community/reborn builds
  ...
  Nintendulator-Extended note:
  This copy is adapted for RUNTIME DYNAMIC LOADING (LoadLibrary /
  GetProcAddress).  It intentionally contains no __declspec(dllimport)
  declarations - the emulator never links against kailleraclient.lib.
  The kailleraInfos structure layout and the callback signatures are
  byte-identical to the official SDK header, so this header is fully
  compatible with every existing DLL build.
*/
...
extern "C" {
  ...
  */
  typedef int (WINAPI *kailleraGetVersionFunc)(char *version);
  /*
     kailleraInit:
     Call this method when your program starts
  */
  typedef int (WINAPI *kailleraInitFunc)();
  /*
     kailleraShutdown:
     Call this method when your program ends
  */
  typedef int (WINAPI *kailleraShutdownFunc)();
  /*
     kailleraSetInfos:
  ...
  */
  typedef int (WINAPI *kailleraSetInfosFunc)(kailleraInfos *infos);
  /*
     kailleraSelectServerDialog:
     Use this method for launching the Kaillera server dialog
  */
  typedef int (WINAPI *kailleraSelectServerDialogFunc)(HWND parent);
  /*
     ...
     Basically, each player sends his values and receives all players values concatenated.

     e.g:
     t   - player 1 sends "867F"            - player 2 sends "964F"
     t+1 - player 1 receives "867F964F"     - player 2 receives "867F964F"

     YOUR input values must be placed at the START of the buffer (offset 0).
     On return, the buffer contains all players' inputs concatenated:
     player i's (1-based) input is located at offset (i-1)*size.
     ...
     Be sure that the values parameter has enough space for receiving all inputs from all players
     (8 players max.)

     returns: length received or -1 on network error (player no more in the game)
              (0 during the initial connection delay setup stage)
  */
  typedef int (WINAPI *kailleraModifyPlayValuesFunc)(void *values, int size);
  /*
      kailleraChatSend
      Use this function to send a line of chat text during a game
  */
  typedef int (WINAPI *kailleraChatSendFunc)(char *text);
  /*
      kailleraEndGame:
      Your emulation thread must call this method when the user stops the emulation
  */
  typedef int (WINAPI *kailleraEndGameFunc)();
